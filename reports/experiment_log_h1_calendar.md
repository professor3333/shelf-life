...
|---|---|---|---|---|---|---|---|
| 01-prior | does anything at all beat a constant? | 0.0077 | 0.0075 | 0.0141 | 0.0124 | -0.0017 | 0.0017 |
| 02-logistic | is there linear signal in the allowed features? | 0.0267 | 0.0153 | 0.0227 | 0.1207 | 0.098 | 0.2047 |
| 03-random_forest | does non-linearity buy anything over the logistic fit? | 0.199 | 0.1024 | 0.1415 | 0.9057 | 0.7642 | 0.0781 |
| 04-xgboost_panel_native | boosting, on the features that existed before any were engineered | 0.1472 | 0.0853 | 0.08 | 0.6895 | 0.6095 | 0.0506 |
| 05-xgboost_engineered | do the seven engineered features earn their place? | 0.0875 | 0.0154 | 0.084 | 0.8001 | 0.7161 | 0.0409 |
| 06-xgboost_leaky | what happens when a panel-wide observation count is admitted? | 0.3015 | 0.0667 | 0.2018 | 0.9921 | 0.7903 | 0.0123 |
| 07-xgboost_leak_removed | and what is left once it is taken away again? | 0.0875 | 0.0154 | 0.084 | 0.8001 | 0.7161 | 0.0409 |
| 08-xgboost_tuned | does tuning inside the training window buy anything? | 0.1959 | 0.0986 | 0.0876 | 0.9086 | 0.821 | 0.028 |
...
- run 05, honest features: **0.0840** validation PR-AUC
- run 06, leaky columns admitted: **0.2018**
- run 07, leaky columns removed: **0.0840**

The leak was worth **+0.1178** PR-AUC, and removing it returned
...
margin. The +0.1178 jump is a single observation that something changed when two
...
+0.1178 *attributable* rather than merely observed: without it I have a
...
0.0840 — a stochastic learner has no obligation to land on the same four
...
too well offline. The expected story is degradation: 0.2018 offline becomes
...
happens here is that it does not decay toward the honest 0.0840 — it stops
...
0.0123, against 0.0409 for the honest run. The only
...
- git `55652f321261997982a9627b1deef7808ce0874f` on `fix/h1-rehearsal-keeps-its-own-ladder-reports`
- panel `data/processed/features/job_days_h1_calendar.parquet`, 21,875 rows, sha256 `7e469b9fe5b6d43f522395522f38fa7136deb27a669bff32106d14ef62693440`
- snapshot `2026-09-19`
