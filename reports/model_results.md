...
The constant-predictor reference remains PR-AUC **0.0116**
on 19,500 labelled rows.
...
| age_ceiling | the best any age-only rule could do (deciles, non-monotone) | 0.0204 | 0.0199 | 0.4653 | 0.0196 | 0.0851 |
| age_only | age_days alone, logistic | 0.0206 | 0.2492 | 0.5261 | 0 | 0 |
| board_hazard | per-board historical rate | 0.0212 | 0.0199 | 0.5072 | 0 | 0 |
| logistic | logistic regression, all allowed features | 0.0271 | 0.1424 | 0.5419 | 0.05 | 0.0426 |
| decision_tree | decision tree, depth-limited | 0.0279 | 0.1687 | 0.6053 | 0.0488 | 0.0426 |
| random_forest | random forest, 300 trees | 0.11 | 0.0272 | 0.602 | 0.1 | 0.0851 |
| xgboost | gradient boosting | 0.0736 | 0.0279 | 0.5884 | 0.075 | 0.0638 |
...
|---|---|---|---|---|---|---|
| nothing | 0 | — | 0.0736 | 0 | 0.8001 | 0.7265 |
| title_seniority | 1 | how senior a role is relates to how long it takes to fill | 0.0704 | 0.0032 | 0.7562 | 0.6858 |
| title_is_manager | 1 | management roles have longer hiring processes | 0.0706 | 0.003 | 0.7856 | 0.715 |
| title_words | 1 | terse titles are boilerplate on high-volume reqs and churn faster | 0.0712 | 0.0024 | 0.804 | 0.7327 |
| title_chars | 1 | as title_words, by another measure | 0.0713 | 0.0023 | 0.7203 | 0.649 |
| location_is_remote | 1 | remote roles draw a larger pool and close faster | 0.072 | 0.0016 | 0.7896 | 0.7176 |
| n_locations | 1 | a posting open in several places is a wider net | 0.0726 | 0.0011 | 0.7992 | 0.7267 |
| salary_band | 1 | pay level relates to fill speed, non-linearly | 0.0724 | 0.0013 | 0.7645 | 0.6922 |
| board_size_at_t | 1 | a posting on a large board competes with more of them | 0.0825 | -0.0088 | 0.7937 | 0.7112 |
| board_growth | 1 | a board that is growing is hiring, and hiring boards close reqs | 0.0772 | -0.0036 | 0.7297 | 0.6525 |
| n_same_title_on_board | 1 | a title duplicated across the board is a high-volume req and churns | 0.0719 | 0.0018 | 0.8188 | 0.7469 |
| n_same_req_on_board | 1 | one requisition posted in several places closes when any of them does | 0.0729 | 0.0008 | 0.8022 | 0.7293 |
| board context (all four) | 4 | board context is worth having at all — what `docs/design.md` §12 was decided on, and the closest a refit gets to what a caller actually loses | 0.0935 | -0.0199 | 0.7557 | 0.6622 |

### What this settles about board context (`docs/design.md` §12)

Withholding all four board columns costs **-0.0199** validation PR-AUC. The largest single board column is worth 0.0088.
...
|---|---|---|---|---|---|---|
| board context supplied | 2320 | 0.0736 | 0.0279 | 0.075 | 0.0638 | 0 |
| absent, imputed | 2320 | 0.0611 | 0.0783 | 0.0488 | 0.0426 | 0.0125 |
...
|---|---|---|---|---|---|---|---|---|
| stump, heavy shrinkage | 1 | 50 | 10 | — | — | 0.0489 | 0.0278 | 0.021 |
| moderate | 4 | 400 | 1 | — | — | 0.8001 | 0.0736 | 0.7265 |
| deep, unregularised | 12 | 1200 | 0 | 1 | 1 | 0.9996 | 0.0912 | 0.9084 |
| deep + min_child_weight | 12 | 1200 | 0 | 30 | 1 | 0.9913 | 0.102 | 0.8893 |
| deep + lambda | 12 | 1200 | 50 | 1 | 1 | 0.9994 | 0.1122 | 0.8872 |
| deep + subsampling | 12 | 1200 | 0 | 1 | 0.6 | 0.9993 | 0.1111 | 0.8883 |
