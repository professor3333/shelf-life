...
|---|---|---|---|---|---|---|---|
| 01-prior | does anything at all beat a constant? | 0.0189 | — | 0.0009 | 0.0173 | 0.0164 | 0.0164 |
| 02-logistic | is there linear signal in the allowed features? | 0.024 | — | 0.0222 | 0.1372 | 0.115 | 0.2096 |
| 03-random_forest | does non-linearity buy anything over the logistic fit? | 0.0615 | — | 1 | 0.8891 | -0.1109 | 0.1344 |
| 04-xgboost_panel_native | boosting, on the features that existed before any were engineered | 0.0709 | — | 0.2 | 0.7193 | 0.5193 | 0.1027 |
| 05-xgboost_engineered | do the seven engineered features earn their place? | 0.0661 | — | 1 | 0.8859 | -0.1141 | 0.0795 |
| 06-xgboost_leaky | what happens when a panel-wide observation count is admitted? | 0.2942 | — | 1 | 0.9945 | -0.0055 | 0.0003 |
| 07-xgboost_leak_removed | and what is left once it is taken away again? | 0.0661 | — | 1 | 0.8859 | -0.1141 | 0.0795 |
| 08-xgboost_tuned | does tuning inside the training window buy anything? | 0.0661 | — | 1 | 0.8859 | -0.1141 | 0.0795 |
...
- run 05, honest features: **1.0000** validation PR-AUC
- run 06, leaky columns admitted: **1.0000**
- run 07, leaky columns removed: **1.0000**

The leak was worth **+0.0000** PR-AUC, and removing it returned
...
margin. The +0.0000 jump is a single observation that something changed when two
...
+0.0000 *attributable* rather than merely observed: without it I have a
...
1.0000 — a stochastic learner has no obligation to land on the same four
...
too well offline. The expected story is degradation: 1.0000 offline becomes
...
happens here is that it does not decay toward the honest 1.0000 — it stops
...
probabilities are well-formed and confident, and run 06 had the best calibration in the whole table — ECE
0.0003, against 0.0795 for the honest run. The only
...
- git `592fb1ea775df46f1dc174d43bae881f0b94ba62` on `chore/reports-2026-09-14`
- panel `data/processed/features/job_days_h1_calendar.parquet`, 16,117 rows, sha256 `614159c15cb031afe90fe4ab1653132361e2b01abd80ae89c4e00afd6a3b376d`
- snapshot `2026-09-14`
