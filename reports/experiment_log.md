...
|---|---|---|---|---|---|
| 01-prior | does anything at all beat a constant? | 0.0034 | 0.0159 | 0.0125 | 0.0125 |
| 02-logistic | is there linear signal in the allowed features? | 0.027 | 0.1488 | 0.1218 | 0.1624 |
| 03-random_forest | does non-linearity buy anything over the logistic fit? | 0.2755 | 0.9216 | 0.646 | 0.1 |
| 04-xgboost_panel_native | boosting, on the features that existed before any were engineered | 0.0674 | 0.96 | 0.8926 | 0.037 |
| 05-xgboost_engineered | do the seven engineered features earn their place? | 0.2539 | 0.9191 | 0.6651 | 0.027 |
| 06-xgboost_leaky | what happens when a panel-wide observation count is admitted? | 0.3267 | 1 | 0.6733 | 0.0021 |
| 07-xgboost_leak_removed | and what is left once it is taken away again? | 0.2539 | 0.9191 | 0.6651 | 0.027 |
| 08-xgboost_tuned | does tuning inside the training window buy anything? | 0.2539 | 0.9191 | 0.6651 | 0.027 |
...
- run 05, honest features: **0.2539** validation PR-AUC
- run 06, leaky columns admitted: **0.3267**
- run 07, leaky columns removed: **0.2539**

The leak was worth **+0.0728** PR-AUC, and removing it returned
...
margin. The +0.0728 jump is a single observation that something changed when two
...
+0.0728 *attributable* rather than merely observed: without it I have a
...
0.2539 — a stochastic learner has no obligation to land on the same four
...
too well offline. The expected story is degradation: 0.3267 offline becomes
...
happens here is that it does not decay toward the honest 0.2539 — it stops
...
0.0021, against 0.0270 for the honest run. The only
...
- git `bfa63a9469238c97b215469b3f2b0f812711c883` on `feature/release-only-from-a-clean-tree` — **dirty tree**
- panel `data/processed/features/job_days_h1_calendar.parquet`, 12,675 rows, sha256 `75873ccaca0f4aba6bb8d2c68a8bfd113af13d0f3449823e12ddf6c20bca50d7`
- snapshot `2026-09-11`
