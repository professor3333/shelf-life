...
|---|---|---|---|---|---|
| 01-prior | does anything at all beat a constant? | 0.0052 | 0.0169 | 0.0117 | 0.0117 |
| 02-logistic | is there linear signal in the allowed features? | 0.0094 | 0.1302 | 0.1208 | 0.1942 |
| 03-random_forest | does non-linearity buy anything over the logistic fit? | 0.1804 | 0.8815 | 0.7011 | 0.1154 |
| 04-xgboost_panel_native | boosting, on the features that existed before any were engineered | 0.0969 | 0.7495 | 0.6526 | 0.1196 |
| 05-xgboost_engineered | do the seven engineered features earn their place? | 0.1743 | 0.8214 | 0.6471 | 0.0722 |
| 06-xgboost_leaky | what happens when a panel-wide observation count is admitted? | 0.3308 | 0.9992 | 0.6684 | 0.0038 |
| 07-xgboost_leak_removed | and what is left once it is taken away again? | 0.1743 | 0.8214 | 0.6471 | 0.0722 |
| 08-xgboost_tuned | does tuning inside the training window buy anything? | 0.1743 | 0.8214 | 0.6471 | 0.0722 |
...
- run 05, honest features: **0.1743** validation PR-AUC
- run 06, leaky columns admitted: **0.3308**
- run 07, leaky columns removed: **0.1743**

The leak was worth **+0.1565** PR-AUC, and removing it returned
...
margin. The +0.1565 jump is a single observation that something changed when two
...
+0.1565 *attributable* rather than merely observed: without it I have a
...
0.1743 — a stochastic learner has no obligation to land on the same four
...
too well offline. The expected story is degradation: 0.3308 offline becomes
...
happens here is that it does not decay toward the honest 0.1743 — it stops
...
0.0038, against 0.0722 for the honest run. The only
...
- git `f8e3c4dd750ecd61a7ae1f2a348b863123bb78a8` on `feature/reports-from-a-clean-tree`
- panel `data/processed/features/job_days_h1_calendar.parquet`, 13,821 rows, sha256 `966888e63fddc8d05a6656e892b30ec700f9385c58f18ca29e2826292bbd25bd`
- snapshot `2026-09-12`
