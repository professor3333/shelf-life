# Test-set results

| | |
|---|---|
| Generated by | `python -m src.models.freeze` |
| Horizon | H=7 (calendar basis) |
| Data | **real** · snapshot `2026-09-14` · `data/processed/features/job_days_h7_calendar.parquet` |
| Panel | 16,117 rows · sha256 `0e645bd1d207…` |
| Code | `592fb1ea7` on `chore/reports-2026-09-14` (clean) |

_Regenerate rather than edit._

**This is the number, whatever it is.** The test block is opened once, after
the pipeline and its threshold are frozen, and nothing downstream of it may
change either. A disappointing figure here is evidence about the validation
discipline, not a licence to tune — the tuning would be selection on test,
and the next number would mean less than this one.

## Nothing below has run

The test set has not been opened, because no honest three-way split exists on
this snapshot yet. That is the correct outcome rather than a failure: a test
block with no positives is not a hard test set, it is an undefined metric, and
freezing a model against one would produce a README number that means nothing.

```
no cut of this panel yields three usable blocks.
```

**How much longer.** 8 labelled crawl wave(s) against a minimum of 22 — 14 more labelled wave(s) needed. The embargo is 9 days 13:43:51.653374 and waves arrive every 0 days 23:59:58.980213, so each of the two block boundaries discards 10 wave(s), and the newest 1 labelled wave(s) cannot carry a positive at all. Legal is not evaluable: the deepest legal cut yields 0 rolling-origin fold(s) today, and 3 fold(s) — the fewest that can show a model losing a fold it was expected to win — need 34 labelled wave(s), 26 more labelled wave(s) needed.

The packaging around it is built and tested — `src/inference/artifact.py`,
`src/inference/contract.py` and `src/inference/predict.py`, exercised end to end
on the synthetic panel — so what waits here is panel depth, not code.


