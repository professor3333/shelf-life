...
**How much longer.** 2 labelled crawl wave(s) against a minimum of 20 — 18 more labelled wave(s) needed. The embargo is 8 days 10:21:37.132005 and waves arrive every 1 days 00:00:03.564294, so each of the two block boundaries discards 9 wave(s), and the newest 1 labelled wave(s) cannot carry a positive at all. Legal is not evaluable: the deepest legal cut yields 0 rolling-origin fold(s) today, and 3 fold(s) — the fewest that can show a model losing a fold it was expected to win — need 31 labelled wave(s), 29 more labelled wave(s) needed.
