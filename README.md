...
state is the block below, written by `python -m src.data.readme_summary` from
those reports and held equal to them by a test — the one place in this file
...
**The current snapshot's shape** is the latest dated profile under
`reports/` (linked from the generated block above): postings, observations,
crawl runs and the window, with the missingness and distribution figures
beside it. The H=7 panel built from it — rows, labelled rows, positives, base
rate, per-board counts — is in [`reports/readiness.md`](reports/readiness.md),
[`reports/label_validity.md`](reports/label_validity.md) and
[`reports/cohort_audit.md`](reports/cohort_audit.md), each regenerated from a
clean tree on the snapshot it names. This section used to carry those tables
for one snapshot, and they were a week stale within a week.
...
**What the real runs have found** is in
[`reports/model_comparison.md`](reports/model_comparison.md), regenerated by
`scripts/rehearse.sh` on every run, and summarised in the generated block at
the top of this file — not restated here, because an earlier version of this
paragraph described a table that a later run no longer supported. What the
table shows today, and will keep showing until the folds exist, is the shape
that matters: single validation draws with no error bar, some of them large,
and a verdict of **nothing selected**. A large single-draw number is not a
selection; with zero folds there is nothing to compare it against, and naming
the leader would be selection on the validation block. That refusal is the
machinery working, and the number beside it is what it is refusing to act on.

**The leak reproduces on real data**, not only on the fixture. On the
2026-09-09 run, run 06 admitted the panel-wide aggregates and scored 0.0290;
run 07 removed them and returned to 0.0226 — the same number as run 05, which
differs in nothing else. The leak was worth +0.0064 PR-AUC that day, about a
quarter of the honest score; the current pair is in
