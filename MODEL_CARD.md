...
|---|---|
| Validation | Pending: baseline and selected PR-AUC (implemented as average precision), fold mean/SD, precision, recall, F1, and TP/FP/FN/TN at the selected threshold. Accuracy is not the headline; ROC-AUC is secondary. |
| Held-out test | Pending: the same metrics, with threshold-dependent results at the frozen validation threshold distinguished from a recomputed test-budget diagnostic. |
| Bootstrap confidence intervals | Pending. Existing test protocol: 95% percentile intervals, 2,000 draws, seed 0; resample `(source, source_id)` posting clusters with all their rows, keeping the model and threshold fixed. Report PR-AUC, precision, recall, Brier and ECE intervals, retained resample counts and the fewer-than-30-positive fragility flag. Zero-positive draws are discarded. These intervals do not measure model-selection uncertainty or unseen-board shift. Validation bootstrap intervals are not currently emitted. |
| Calibration | Pending: Brier, ECE and the ten equal-width-bin reliability table, including bin counts. No final calibration quality or fitted recalibration method has been established. |
| Per-board / cohort performance | Pending: each eligible board, incident versus incumbent stock, first-observation-only rows, and carried-over versus unseen-in-training postings; include sample sizes, positive counts and unavailable/undefined metrics explicitly. |
| Unseen-board transfer | Pending: leave-one-board-out score, board base rate, comparison with the board included in training, and skipped-board reasons. Name the actual evaluated family/run: the current transfer driver builds XGBoost, so its eventual result cannot automatically describe another selected family. |
