...
def write_report(
    ...
    lines = [
        "# Experiment history",
        "",
        *provenance.header(prov, "python -m src.models.experiments", banner),
    ...
    if table is not None:
        ...
        lines += [
            ...
            "",
            *in_my_own_words(table).splitlines(),
            "",
        ]
...
#: The one section of this report a generator must not produce. The table above
#: is machinery; this is the reading of it, and the project's learning contract
#: puts it in the author's column precisely because a fluent account of a leak
#: is not evidence that the leak was understood. Kept here rather than in the
#: `.md` because the report is rewritten on every run and hand-edits to it do
#: not survive.
_IN_MY_OWN_WORDS = """### In my own words
...
**Which number is the stronger evidence.** The **{restored:.4f}**, and not by a small
margin. The +{inflation:.4f} jump is a single observation that something changed when two
...
The {restored:.4f} is the counterfactual. It says runs 05 and 07 are the same experiment
...
+{inflation:.4f} *attributable* rather than merely observed: without it I have a
...
{honest:.4f} — a stochastic learner has no obligation to land on the same four
...
too well offline. The expected story is degradation: {leaked:.4f} offline becomes
...
happens here is that it does not decay toward the honest {honest:.4f} — it stops
...
probabilities are well-formed and confident, and {calibration_clause} — ECE
{leaked_ece:.4f}, against {honest_ece:.4f} for the honest run. The only
...
def in_my_own_words(table: pd.DataFrame) -> str:
    """The author's account of the leak, with this run's numbers in it.

    The words are the author's and stay that way; only the numerals are
    substituted. They used to be typed into the prose, which was fine until the
    runs were replayed on the real panel and the paragraphs went on quoting a
    synthetic replay — a report asserting +0.6471 three lines under a table
    showing +0.7186. Two numbers for one quantity in one file is worse than
    either alone, because a reader has no way to tell which is the stale one.

    Everything here is read from `table`, so the account cannot drift from the
    evidence above it again.
    """
    delta = leak_delta(table)
    ece = table.set_index("run")["val_ece"]
    leaked_ece = float(ece["06-xgboost_leaky"])

    # The superlative is checked rather than asserted. On the synthetic replay
    # the leaky run *was* the best-calibrated in the table, which is the sharpest
    # form of the point: the one diagnostic a reader would reach for to catch a
    # broken model rates it the best of the lot. On another panel that may not
    # hold, and a sentence that keeps claiming it would be a false statement
    # sitting directly above the table that refutes it — the exact failure this
    # function exists to end, reintroduced one line higher up.
    clause = (
        "run 06 had the best calibration in the whole table"
        if leaked_ece <= ece.min()
        else "run 06 is as well calibrated as the honest run"
    )
    return _IN_MY_OWN_WORDS.format(
        calibration_clause=clause,
        restored=delta["restored"],
        inflation=delta["inflation"],
        honest=delta["honest"],
        leaked=delta["leaked"],
        leaked_ece=leaked_ece,
        honest_ece=float(ece["05-xgboost_engineered"]),
    )
...
def main() -> None:
    ...
    write_report(args.out, table, prov, blocker, horizon_banner(panel))
